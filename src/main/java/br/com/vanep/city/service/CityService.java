...
public class CityService {
  ...
  public Page<CityResponseDTO> findByUf(String uf, String search, Pageable pageable) {
    ...
    if (search == null || search.isBlank()) {
      return cityRepository.findByStateId(state.getId(), pageable).map(mapper::toResponse);
    }
    String normalizedSearch = LocationNameNormalizer.normalize(search);
    return cityRepository
        .findByStateIdAndNormalizedNameContaining(state.getId(), normalizedSearch, pageable)
