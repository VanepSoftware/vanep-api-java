...
import br.com.vanep.media.mapper.MediaFileMapper;
...
public class MediaFileService {
  ...
  private final MediaOwnerResolver ownerResolver;
  ...
  public MediaFileService(
      ...
      MediaOwnerResolver ownerResolver,
      MimeTypeDetector mimeTypeDetector,
      MediaFileMapper mapper,
  ...
  public MediaFileResponseDTO upload(
      MediaOwnerType ownerType, String ownerToken, MediaPurpose purpose, MultipartFile file) {
    Long ownerId =
        ownerResolver
            .resolveOwnerId(ownerType, ownerToken)
            .orElseThrow(() -> notFound("media.owner.not_found"));

    if (file == null || file.isEmpty()) {
      throw badRequest("media.file.required");
    }
    if (file.getSize() > maxSizeFor(purpose)) {
    ...
    media.setObjectKey(objectKey(ownerType, ownerToken, purpose, media.getToken(), mimeType));
    media.setMimeType(mimeType);
    media.setSizeBytes((long) content.length);
    media.setOriginalName(file.getOriginalFilename());
    media.setVisibility(purpose.defaultVisibility());

    storage.upload(
        media.getObjectKey(), new ByteArrayInputStream(content), mimeType, content.length);

    return mapper.toResponse(repository.save(media));
  }

  @Transactional(readOnly = true)
  public MediaFileResponseDTO findByToken(String token) {
    return mapper.toResponse(requireMedia(token));
  }
  ...
  private String objectKey(
    ...
    return "%s/%s/%s/%s.%s"
        .formatted(
            ownerType.name(), ownerToken, purpose.name(), mediaToken, extensionFor(mimeType));
  }
  ...
  private long maxSizeFor(MediaPurpose purpose) {
    return purpose == MediaPurpose.DOCUMENT ? maxDocumentBytes : maxPhotoBytes;
  }
