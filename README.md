...
> Nomes de arquivos, classes, pacotes e identificadores são sempre em **inglês** — nunca pt-BR. Só mensagens ao usuário e descrições de commit/PR ficam em português.
...
| **Resource Server** | Rotas **`/api/**`** são protegidas por **JWT** (Bearer). Sem token → **401**. Ex.: `GET /api/user/me` devolve o perfil da conta autenticada (consumido pelo front como "userinfo"). |
