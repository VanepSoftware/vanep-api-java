...
### D1 — `status` é `PENDING`, `ACTIVE`, `INACTIVE`, `BLOCKED`

Os três primeiros vêm direto do `Enum relationship_status` do DBML. O significado que o canônico dá: o vínculo **nasce `PENDING`** no primeiro envio de proposta, vira `ACTIVE` quando existe contrato ativo, e `INACTIVE` quando a relação encerra. Esta change não implementa essas transições — não há proposta nem contrato — mas o enum nasce completo para que a #42 não precise de migration.
...
**`BLOCKED` entra por pedido de revisão na #198**, e é o único valor que não está no DBML — então **o diagrama precisa acompanhar**, senão o canônico e o código divergem, que é exatamente a dívida que a change `rating-through-link` está pagando.

Ele não é sinônimo de `INACTIVE`. `INACTIVE` é uma relação que acabou e pode recomeçar; `BLOCKED` é uma que **não pode** — nenhuma proposta nova deve ressuscitar o par.

Entra agora pelo mesmo argumento acima: o valor é barato hoje e caro depois. Mas entra **declaradamente incompleto**, e o que falta está no D9.

### D9 — `BLOCKED` ainda não diz quem bloqueou quem

Um `status` sozinho não carrega direção. "O cliente bloqueou o motorista" e "o motorista bloqueou o cliente" são fatos diferentes, e com um valor só os dois viram a mesma linha.

Isso vaza para a autorização que esta change introduz: `@sec.isClientDriverLinkParty` dá leitura às **duas** partes, então hoje quem foi bloqueado enxerga o vínculo e vê o próprio bloqueio. E não há como responder "quem desbloqueia?" — a resposta correta é *só quem bloqueou*, e essa informação não existe no modelo.

Resolver exige `blocked_by`, ou duas colunas, ou tabela própria — e exige também alguém que **aplique** o bloqueio, isto é, a #42 recusando proposta para um par bloqueado. Nada disso existe.

Decisão: o valor entra, a semântica fica registrada como pendência (R8), e a direção vem junto com a #42, que é quem move o `status`. Acrescentar `blocked_by` agora seria uma coluna que ninguém escreve e ninguém lê.
...
| **R8** | `BLOCKED` entra sem direção e sem quem o aplique: ninguém sabe quem bloqueou quem, e nada recusa proposta para par bloqueado | Registrado no D9. `blocked_by` e a recusa vêm com a #42; até lá o valor é aceito e persistido, mas não produz efeito |
...
| **Q3** | Quem pode desbloquear um vínculo `BLOCKED`? | 🔴 Decidir na #42. A resposta correta é *só quem bloqueou*, e o modelo ainda não guarda isso (D9) |
