...
`RelationshipStatus` MUST be a backed Java enum with the values `PENDING`, `ACTIVE` and `INACTIVE`, matching `relationship_status` in `vanep-diagram.dbml`. A link created without an explicit status MUST default to `PENDING`.
