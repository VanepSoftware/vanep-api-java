...
- Vínculo: motorista convida por e-mail → e-mail real (MailService/Mailpit) → página Thymeleaf de aceite/recusa → pause/resume/revoke
- Remoção completa do desenho `driver_link_code` / generate / cancel / consume / `linkCode` no signup
- Testes unitários e de integração

**Fora de escopo:**
- SMS / push de convite
- Aceite/recusa via REST autenticado (só formulário Thymeleaf no MVP; REST fica backlog se o app nativo precisar)
...
- `assistant-linking`: Convite por e-mail (`assistant_invite`), página web de aceite/recusa, pause/resume/revoke, cooldown pós-recusa
