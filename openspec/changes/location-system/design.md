...
|---|---|---|---|
| **Q1** | O faturamento por sessão sobrevive a chaves distintas (autocomplete com referrer, `Place Details` com servidor)? | 🟡 **parcial — não verificável ainda** | A doc confirma que o SKU **Autocomplete Session Usage custa US$ 0, ilimitado**, e que a sessão só entra nele se o `Place Details` que a encerra carregar o mesmo `sessionToken`. Isso *eleva* a importância do D5: sem o repasse, cada tecla vira `Autocomplete Requests` (10k grátis, depois US$ 2,83/1.000). **O que falta:** confirmar que a fronteira entre chaves não quebra a sessão. Isso só é observável quando existir tráfego real de autocomplete vindo da chave web, o que exige o frontend implementado — não há como fechar antes disso. Até lá, a consequência é apenas de custo, e o pior caso (autocomplete cobrado avulso) tem 10.000 chamadas grátis por mês |
...
| **Q3** | Qual a ordem de grandeza do custo por busca? | ✅ **resolvida** | Relatório de billing **zerado** após o spike: as ~26 chamadas ficaram inteiramente dentro da cota gratuita, como a tabela previa. 1 busca = 2 `Place Details` = 2 eventos do balde de 10.000/mês → **~5.000 buscas/mês grátis**; acima disso, **US$ 0,01 por busca**. 1 endereço salvo = 1 evento; 1 escola resolvida = 1 evento mais o SKU Pro (Q6). Quota diária de 300 definida no console, o que mantém o projeto dentro do gratuito por construção |
...
**Q2, Q3, Q4 e Q5 estão fechadas.** A Q6 foi decidida na fase 7 (pagar o SKU Pro pelo `displayName`, porque o custo é limitado pelo número de escolas e o nome é rótulo de recurso compartilhado).

Sobra a **Q1**, e ela não é fechável agora: depende de tráfego real de autocomplete saindo da chave web, o que só existe com o frontend implementado. Fica registrada como item a verificar no primeiro relatório de billing depois que o autocomplete entrar no ar — procurar a linha **Autocomplete Session Usage** (US$ 0). Se em vez dela aparecer só `Autocomplete Requests`, a sessão quebrou na fronteira entre chaves e o D5 precisa de reação. Não bloqueia a v1: o pior caso é custo, com 10.000 chamadas grátis por mês de folga.
