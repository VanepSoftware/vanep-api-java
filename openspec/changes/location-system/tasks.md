...
- [x] 0.5 Criar a **chave web** (vanep-frontend), restrita por **HTTP referrer**, limitada a Places API (New) — em `vanep-frontend/.env` como `GOOGLE_PLACES_API_KEY`, distinta da de servidor. Verificada: sem `Referer` e com referrer estranho devolve `403 API_KEY_HTTP_REFERRER_BLOCKED`, com `http://localhost:3000/` devolve `200`. Falta acrescentar o domínio de produção quando existir
