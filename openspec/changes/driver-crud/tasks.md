...
### Phase 2 — REST API
- [x] 2.1 Create DTOs: `DriverResponseDTO` and `DriverUpdateRequestDTO` in `br.com.vanep.driver.dto`. Include validation annotations on the request DTO.
- [x] 2.2 Create `DriverMapper` in `br.com.vanep.driver.mapper` to handle entity-DTO mapping.
- [x] 2.3 Create `DriverSecurityService` in `br.com.vanep.driver.security` for ownership check. (Note: implemented as global SecurityEvaluator in br.com.vanep.auth.security).
- [x] 2.4 Create `DriverService` in `br.com.vanep.driver.service` (methods: `findAll(Pageable)`, `findByToken(String)`, `update(String, DriverUpdateRequestDTO)`, `delete(String)`, `restore(String)`).
- [x] 2.5 Create `DriverController` in `br.com.vanep.driver.controller` with routes `/api/drivers/**` and security annotations.
- [x] 2.6 Validate using `./mvnw verify`.
- [ ] 2.7 Open PR2: `feat(driver): phase 2 — REST API` -> `main`.

### Phase 3 — Seed & Tests
- [x] 3.1 Extend `src/test/resources/db/clean.sql` with `delete from driver;` (Note: was already present in clean.sql).
- [x] 3.2 Add mock drivers to `DataSeeder.java` to allow testing list pagination and different status filters.
- [x] 3.3 Create `DriverServiceTest` (unit test covering edge cases: not found, updating restricted fields, deleting and restoring).
- [x] 3.4 Create `DriverControllerTest` (MockMvc integration tests covering all HTTP endpoints, roles, and ownership authorization).
