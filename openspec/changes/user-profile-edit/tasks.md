...
- [ ] 3.1 Failing unit tests for `UserProfileService.patchMe` (absent no-op, null→400, blank phone→400, name cooldown→409, gender always ok, same value no cooldown bump)
- [ ] 3.2 Create `UserProfileUpdateRequestDTO` with `JsonNullable` fields
- [ ] 3.3 Implement `UserProfileService.patchMe` (load via `UserService.requireByToken`, apply policy, persist `last_name_change_at` / `last_phone_change_at` only when value changes)
- [ ] 3.4 Add `PATCH /api/user/me` on `ProfileController` with `@Valid` + `isAuthenticated()`
- [ ] 3.5 MockMvc slice tests: 401, 200 happy path, 400 blank phone, 409 name cooldown with body shape (`code=cooldown`, `retryAfter` ISO)
