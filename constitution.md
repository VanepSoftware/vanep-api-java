...
52. **Run `make lint` at the end of every task, and again immediately before every `commit` and every `push`** — not just before opening a phase PR (rule 44 already covers that narrower, PR-scoped case). `make lint` runs `./mvnw spotless:check`, which only verifies formatting (Google Java Format), unused-import removal, and trailing whitespace — it is **not** a static analyzer and will not flag unused private methods/fields, dead code, or other IDE-only hints (those come from the editor's language server, which frequently misreports reflection-invoked methods like `@PrePersist`/`@BeforeEach` callbacks as "unused" since it cannot see the framework calling them). If `make lint` fails, fix it with `make lint-fix` and re-run `make lint` before proceeding; never commit or push on a red lint.
